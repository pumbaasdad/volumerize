...
function discoverDatabases() {
  local x
  for ((x = 1; ; x++)); do
    local VARIABLE_DB_HOST="VOLUMERIZE_MYSQL_HOST${x}"
...
function prepareDBConfiguration() {
  ...
  local VARIABLE_DB_HOST="VOLUMERIZE_MYSQL_HOST${jobNumber}"
  local VARIABLE_DB_PASSWORD="VOLUMERIZE_MYSQL_PASSWORD${jobNumber}"
  local VARIABLE_DB_USERNAME="VOLUMERIZE_MYSQL_USERNAME${jobNumber}"
  local VARIABLE_DB_DATABASE="VOLUMERIZE_MYSQL_DATABASE${jobNumber}"
  local VARIABLE_DB_SOURCE="VOLUMERIZE_MYSQL_SOURCE${jobNumber}"
  ...
  check_env ${jobType} ${VARIABLE_DB_HOST} ${VARIABLE_DB_PASSWORD} ${VARIABLE_DB_USERNAME} ${VARIABLE_DB_DATABASE} ${VARIABLE_DB_SOURCE}
...
function prepareDatabase() {
  local jobNumber=$DB_ID
  local jobType=$MYSQL_JOB_TYPE
  HOST_VARIABLE="VOLUMERIZE_MYSQL_HOST${jobNumber}"
...
function databaseExecution() {
  if [ -n "${VOLUMERIZE_MYSQL_HOST}" ] || [ -n "${DB_ID}" ]; then
    prepareDatabase "$@" || returnCode=$? && true ;
    if [ "$returnCode" -gt 0 ]; then
